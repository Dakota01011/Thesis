...
bool FPGAHelper::activateDMA(UINTPTR BuffAddr, u32 Length) {
	...
	if (Length <= MaxLength)
	{
		...
		while((XAxiDma_ReadReg(XPAR_AXI_DMA_0_BASEADDR, XAXIDMA_CR_OFFSET) & XAXIDMA_CR_RESET_MASK) == 1)
		{}
		...
		while((XAxiDma_ReadReg(XPAR_AXI_DMA_0_BASEADDR, XAXIDMA_SR_OFFSET) & XAXIDMA_IDLE_MASK) == 0)
		{}
	}
	else
	{
